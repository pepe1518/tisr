...
class solucionario
{
	function nuevo(){
		...
		$template -> SetTemplate('tpl/form_solucionario.html');
	...
	function guardar(){
		...
		if($todoOK) {
			...
			if($query -> dbInsert($arreglo,"solucionario")){
		...
		echo "<script>window.location.href='solucionario.php'</script>";
	...
	function eliminar(){
		$query = new query;
		if($query -> dbDelete('solucionario','where id_solucionario='.$_GET['id'])){
	...
	$template -> SetParameter('titulo','Solucionarios de los problemas');
		$lista = "";
		/*Recuperar de la BD*/
		$query = new query;
		$listaproveedor = $query -> getRows("*","solucionario","");
			...
			$problema= $query->getRow('nombre_problema','problema','where id_problema= '.$valor['id_problema']);
			 ...
			 $lista .= '<tr>
						 ...
						 <td><a  href="'.$valor['archivo_solucionario'].'"  TARGET="_blank" ><center><img src="images/pdf.png" /></center></a></td>
							<td><a href="#" onclick="ajax(\'formulario_nuevo\',\'solucionario.php?action=editar&id='.$valor['id_solucionario'].'\',\'\'); return false;"><center><img src="images/edit.gif" /></center></a></td>
							<td><a onclick="return confirm(\'Esta seguro de eliminar los datos?\');" href="solucionario.php?action=eliminar&id='.$valor['id_solucionario'].'"><center><img src="images/delete.gif" /></center></a></td>
						   </tr>';
			}
			$lista .= "</table>";
			//$lista .= Paginador(count($listaproveedor),"solucionario.php",20);
		} else {
			$lista = "No se tienen datos registrados";
		}
		$template -> SetParameter('lista',$lista);
		$template -> SetParameter('archivo','solucionario.php');
		return $template -> Display();
	}
