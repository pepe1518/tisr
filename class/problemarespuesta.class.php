		...
		$arreglodatos= $_POST['observaciones'];
			...
			if ($upload -> UploadFile($nombrearchivosubido.".".$upload->ext))
			  ...
			  $arreglo['respuesta_archivo'] = $rutasubida."/".$nombrearchivosubido.".".$upload->ext;
			    $respuestaarhisvos= $rutasubida."/".$nombrearchivosubido.".".$upload->ext;
		...
		if($todoOK)
        {
         ...
         $vandera=false;
           ...
           // echo "ok";
            ...
            {
            	$arreglo['error_compilacion']="Error Sintaxtico";
            	 $error=  substr($result[1], 51, -1);
            		echo "<script>alert('Datos registrados exitosamente  $error ');</script>";
            }
            
            $error=  substr($result[1], 51, -1);
            echo $error;
             /**
              $idlenguaje=	$_POST['id_lenguaje'];
              $idproblema=$_SESSION['idproblemas'];
              $idusuario=$_SESSION['idusuario'];
              $nombrearchov=$_FILES['imagen_path']['name'];
              */
             
 //  $sql='INSERT INTO respuesta(id_lenguaje, id_problema, id_usuario, respuesta_nombre, error_compilacion, respuesta_archivo, resultado) VALUES (1, 2, 6, NULL, '.$error.', NULL, '');';
       ...
       if($vandera==true)
		...
		$datossalidad=array();
			...
			echo "<script>alert('Datos registrados exitosamente <?php  ?>');</script>";
		} else {
	...
	function lista(){
		...
		$template -> SetTemplate('tpl/form_problemarespuesta.html');
